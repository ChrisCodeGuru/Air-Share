...
func DeleteFolder(w http.ResponseWriter, r *http.Request, db *sql.DB) {

	// Get route variables
	folder_id := mux.Vars(r)["id"]
	// Validate id is acceptable
	matched, _ := regexp.MatchString(`^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`, folder_id)
	if !matched {
	...
	if err != nil {
		...
		w.Write([]byte("you are not authorised to delete this folder"))
		return
	}
	...
	counterr := db.QueryRow("SELECT COUNT(*) FROM folder WHERE id=$1", folder_id).Scan(&count)
	if counterr != nil {
	...
	// Check if user has permissions to delete the folder (is owner)
	var permission int

	filepermissionerr := db.QueryRow("SELECT permission FROM account_folder WHERE account_id=$1 AND folder_id=$2", JWTClaims.UUID, folder_id).Scan(&permission)
	...
	deleteerr := db.QueryRow("SELECT delete_folder($1)", folder_id).Scan(&foldername)
	if deleteerr != nil {
		// throw 500 internal server error
		w.WriteHeader(http.StatusInternalServerError)
	...
	fileDetailsJSON, err := json.Marshal(folderDetails)
	if err != nil {
