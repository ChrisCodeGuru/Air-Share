...
func Username(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	if len(r.Header["Referer"]) == 0 || (r.Header["Referer"][0] != "https://localhost/settings" && r.Header["Referer"][0] != "https://localhost/fileshare" && r.Header["Referer"][0] != "https://localhost/") || len(r.Header["X-Csrf-Token"]) == 0 {
		w.WriteHeader(401)
		fmt.Println("CSRF")
		...
		var username string
		tokenerr := db.QueryRow("SELECT username FROM account WHERE id = $1", JWTClaims.UUID).Scan(&username)
		if tokenerr != nil {
			fmt.Println(tokenerr)
		}
		w.WriteHeader(200)
