...
// Login API
func LoginMFA(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	passworderr := db.QueryRow("SELECT id, password, verified, otp_secret FROM account WHERE email=$1 OR username=$1;", loginUser.Username).Scan(&uuid, &hashed_password, &verified, &otp_secret)
	if passworderr != nil {
	...
	if subtle.ConstantTimeCompare(originalArgon2Hash, newArgon2Hash) == 1 {
		...
		// verify mfa token
		valid := totp.Validate(loginUser.Token, otp_secret)
		if !valid {
			// throw 401
			fmt.Println(loginUser.Token, otp_secret)
			w.WriteHeader(http.StatusUnauthorized)
